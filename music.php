...
<html lang="en">
	...
	<body>
		...
		<!-- Subcategories: Feed (poll), Discover (discover), Hip-Hop (hip-hop), Pop (pop), EDM (edm),
		...
		<script>
			...
			$(function() {
				$("#edm").on("click", function() {
					window.location.replace("music.php?s=edm");
				});
			});
			$(function() {
				...
				if(isset($_GET["s"])) {
					if($_GET["s"] == "feed") {
						...
						echo "$(function() {
								...
								$(\".edm\").show();
								$(\".subtopic\").css({\"background-color\": \"#fff\", \"color\": \"#000\"});
								$(\"#edm\").css({\"background-color\": \"#FFD700\", \"color\": \"#fff\"});
							});";
					}
					else if($_GET["s"] == "rock") {
						echo "$(function() {
